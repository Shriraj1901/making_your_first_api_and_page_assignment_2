...
// List of status codes and their descriptions
...
// Default route to show instructions
app.get("/", (req, res) => {
  res.send(`
        <h1>Welcome to the HTTP Status Code API</h1>
        <p>Use the <code>/status-info</code> endpoint to query HTTP status codes.</p>
        <p>Example usage: <code>/status-info?code=200</code></p>
        <p>Supported status codes: 200, 201, 204, 400, 401, 403, 404, 405, 429, 500, 502, 503, 504</p>
    `);
});

// HTTP Status Code API endpoint
app.get("/status-info", (req, res) => {
  // Read the query parameter 'code' from the URL
  const { code } = req.query;

  // Check if the code is provided and exists in the statusDescriptions object
  if (code && statusDescriptions[code]) {
    // Respond with the status code and the description
    res.status(Number(code)).json({
      status: Number(code),
      message: statusDescriptions[code],
    });
  } else {
    // If the code is invalid or missing, return a 400 Bad Request error
    res.status(400).json({
      error:
        "Invalid or missing status code. Please provide a valid HTTP status code.",
    });
  }
});
